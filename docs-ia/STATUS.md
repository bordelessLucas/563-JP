...
Atualizado em **2026-09-04**. Fonte operacional para agentes e time: ler junto com `escopo.md`, `mvp_sprints.md` e `checklist_sprints.md`.

## Situação atual

O **MVP demonstrável (MVP-0 → MVP-3)** está **implementado, com pass de UI/UX de fechamento** e **Firestore rules/indexes deployados** no projeto Firebase `jp-6a9d2`.
...
## O que foi aplicado hoje (sessão)
...
2. Checkout completo → pagamento → **Confirmar pagamento**.
...
**Falha mock:** botão “Simular falha” no pagamento **não** cria pedido.
...
| `/admin/products` | Admin | CRUD básico |
| `/admin/categories` | Admin | CRUD básico |
